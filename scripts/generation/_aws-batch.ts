 ...
 * PDF in practice-exams-pdfs-20260508-142418/. Runs scripts/generation/_pdf-to-seed.ts
 * once per PDF with the appropriate config (vendor, slug, code, exam title,
 * domains, level, duration, etc.).
 *
 *   npx tsx scripts/generation/_aws-batch.ts
 *
 * Output: scripts/generated-seeds/seed-aws-<cert>-p<n>.ts (one file per PDF). Skips dupes
...
function buildConfig(job: Job) {
  ...
  return {
    ...
    tag: `manual:${slug}`,
    outputPath: `scripts/generated-seeds/seed-${slug}.ts`,
    defaultDomain: fam.defaultDomain
  };
}

async function main() {
  const tmpCfg = resolve(process.cwd(), 'scripts', 'generation', '_tmp-config.json');
  let ok = 0, fail = 0;
  for (const job of JOBS) {
    const cfg = buildConfig(job);
    writeFileSync(tmpCfg, JSON.stringify(cfg, null, 2), 'utf8');
    try {
      execSync(`npx tsx scripts/generation/_pdf-to-seed.ts ${tmpCfg}`, { stdio: 'inherit' });
      ok++;
    } catch {
